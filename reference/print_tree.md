...
This function prints a hierarchical representation of a package's
dependencies, including version and base status
...
- tree:
  ...
  A nested list representing the package dependency tree, where each
  package has a \`version\` field and potentially sub-dependencies.
...
- last:
  ...
  A logical value indicating if the current package is the last in the
  tree level (default: \`TRUE\`).

- show_version:

  A logical value indicating whether to display package versions if
